        ...
        <div className="bg-white rounded-md py-2 px-2 mt-5">
            ...
            {isLoaded && (<form onSubmit={changeSettings} className="px-6 mb-6 w-100 grid grid-rows-12">
                ...
                <div className="grid grid-cols-2 gap-2">
                    <input data-testid="settings-first-name-input" ref={firstNameRef} className="border pl-[14px] border-gray-400 w-100 py-2 px-1 rounded-md placeholder-gray-700 mt-2 focus:outline-gray-300" placeholder="Имя" defaultValue={currentUserData.user.first_name} required />
                    <input data-testid="settings-last-name-input" ref={lastNameRef} className="border pl-[14px] border-gray-400 w-100 py-2 px-1 rounded-md placeholder-gray-700 mt-2 focus:outline-gray-300" placeholder="Фамилия" defaultValue={currentUserData.user.last_name} required />
                </div>
                <input data-testid="settings-job-title-input" ref={jobTitleRef} className="border pl-[14px] border-gray-400 w-[100%] py-2 px-1 rounded-md placeholder-gray-700 mt-2 focus:outline-gray-300" placeholder="Должность:" defaultValue={currentUserData.job_title} required />
                <textarea data-testid="settings-description-input" ref={descriptionRef} style={{resize: "none"}} className="h-[10em] w-[100%] border pl-[14px] border-gray-400 py-2 px-1 rounded-md placeholder-gray-700 mt-2 focus:outline-gray-300" placeholder="Описание" defaultValue={currentUserData.description}></textarea>
                <div className="flex justify-center mt-2">
                    <button data-testid="change-button" type="submit" className="bg-indigo-700 py-1 px-2 w-[15em] h-[2.5em] rounded-md text-white mt-2 hover:bg-indigo-600">Изменить</button>
                </div>
            </form>)}
            ...
            {successChanged && <span data-testid="changed-successfully" className="h-[20px] py-1 text-sm text-green-500">Настройки были успешно изменены!</span>}
        </div>
    );
};