    ...
    return (
        <form onSubmit={createTeam} className="bg-white rounded-md py-2 px-2">
            ...
            <div className="px-6 mb-6 w-100 grid grid-rows-12">
                <div className="mb-2">
                    <img className="h-[8em] w-[8em] object-cover rounded-full" src={selectedImageSrc ? selectedImageSrc : defaultImage} alt="Current team image" />
                ...
                <input onChange={async () => {await checkTeamName()}} ref={teamNameRef} type="text" className="border pl-[14px] border-gray-400 w-100 py-2 px-1 rounded-md placeholder-gray-700 mt-2 focus:outline-gray-300" placeholder="Название команды:" required />
                ...
                <textarea ref={descriptionRef} type="text" style={{resize: "none"}} className="h-[10em] w-100 border pl-[14px] grid border-gray-400 py-2 px-1 rounded-md placeholder-gray-700 mt-2 focus:outline-gray-300" placeholder="Введите описание задания:"></textarea>
            </div>
            ...
            <div className="text-center mt-2">
                {successCreated && <span data-testid="team-created-span-test" className="h-[20px] py-1 text-sm text-green-500">Команда была успешно создана!</span>}
            </div>
        </form>
    );
};