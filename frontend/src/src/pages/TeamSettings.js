    ...
    return (
        <form onSubmit={changeTeamSettings} className="bg-white rounded-md py-2 px-2">
            ...
            <div className="px-6 mb-6 w-100 grid grid-rows-12">
                ...
                <input data-testid="team-name-input" defaultValue={currentSettings.name} onChange={async () => {await checkTeamName()}} ref={teamNameRef} type="text" className="border pl-[14px] border-gray-400 w-100 py-2 px-1 rounded-md placeholder-gray-700 mt-2 focus:outline-gray-300" placeholder="Название команды:" required />
                {isTeamNameExist && <span data-testid="team-name-exist" className="h-[20px] py-1 text-center text-sm text-red-500">Команда с таким именем уже существует!</span>}
                <textarea data-testid="team-description-textarea" ref={descriptionRef} type="text" style={{resize: "none"}} className="h-[10em] w-100 border pl-[14px] grid border-gray-400 py-2 px-1 rounded-md placeholder-gray-700 mt-2 focus:outline-gray-300" placeholder="Введите описание команды:" defaultValue={currentSettings.description}></textarea>
            </div>
            ...
            <div className="text-center mt-2">
                {successChanged && <span data-testid="changed-successfully" className="h-[20px] py-1 text-sm text-green-500">Настройки команды были успешно изменены!</span>}
            </div>
        </form>
    );
};